  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Overall System Status", value: "Operational", icon: <Globe className="w-5 h-5 text-green-500" />, status: "bg-green-500" },
          { label: "Active Connections", value: "1,294", icon: <Activity className="w-5 h-5 text-blue-500" />, status: "bg-blue-500" },
          { label: "Storage Capacity", value: "42%", icon: <Database className="w-5 h-5 text-amber-500" />, status: "bg-amber-500" },
          { label: "Security Alerts", value: "0", icon: <ShieldAlert className="w-5 h-5 text-green-500" />, status: "bg-green-500" },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${stat.status}`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-black text-primary uppercase tracking-tight">Real-time System Load</h3>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-primary rounded-full animate-pulse" />
              <span className="text-[10px] font-black text-primary uppercase tracking-widest underline decoration-secondary decoration-2 underline-offset-4">LIVE UPDATES</span>
            </div>
          </div>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={performanceData}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="load" stroke="#003366" strokeWidth={4} fillOpacity={1} fill="url(#colorLoad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-primary rounded-[40px] p-8 text-white space-y-6 relative overflow-hidden">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 bg-white/10 rounded-2xl border border-white/5 font-mono text-[10px]">
                <div className="flex items-center space-x-3">
                  <span className="text-secondary font-black">[{log.type}]</span>
...
export const SuperAdminDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { icon: <Users className="w-5 h-5" />, label: "User Management", path: "/super-admin/dashboard/users" },
    
    // Admin Monitor Section
    { label: "Admin Monitoring", isHeader: true },
    { icon: <Building2 className="w-5 h-5" />, label: "Agencies (Admin)", path: "/super-admin/dashboard/admin-agencies" },
    { icon: <FileCheck className="w-5 h-5" />, label: "Applications (Admin)", path: "/super-admin/dashboard/admin-apps" },
    { icon: <ShieldCheck className="w-5 h-5" />, label: "Licenses (Admin)", path: "/super-admin/dashboard/admin-licenses" },
    { icon: <Map className="w-5 h-5" />, label: "GPS Tracking (Admin)", path: "/super-admin/dashboard/admin-gps" },
    { icon: <BarChart3 className="w-5 h-5" />, label: "HRMS Reports (Admin)", path: "/super-admin/dashboard/admin-hrms" },
    
    // System Section
    { label: "System Control", isHeader: true },
    { icon: <History className="w-5 h-5" />, label: "Global Audit", path: "/super-admin/dashboard/audit" },
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} title={t.dashboard.superAdminDashboard}>
      <Routes>
