...
import { FileText, Plus, Trash2, Download, Search, FileUp, Shield, Gavel, X, Eye, Edit2, Check, AlertCircle } from 'lucide-react';
...
export const ManagePublicContent = () => {
  ...
  const [documents, setDocuments] = useState<PublicDocument[]>([
    { 
      id: 1, 
      title: isAm ? "የግል ደህንነት ኤጀንሲዎች መመሪያ" : "Private Security Agency Directive", 
      type: 'Regulation', 
      version: "v2.1", 
      date: "2024-01-15", 
      fileSize: "2.4 MB",
      description: isAm ? "ይህ መመሪያ የግል ደህንነት ኤጀንሲዎችን አሰራር እና አስተዳደር ይወስናል" : "This directive determines the operation and management of private security agencies."
    },
    { 
      id: 2, 
      ...
      date: "2023-11-20", 
      fileSize: "1.1 MB",
      description: isAm ? "የደህንነት ሰራተኞች ሊከተሏቸው የሚገቡ የባህሪ እና የስነ-ምግባር ደንቦች" : "Behavioral and ethical rules that security personnel must follow."
    },
    { 
      id: 3, 
      title: isAm ? "የማመልከቻ ቅጽ - የንብረት ጥበቃ" : "Application Form - Asset Guarding", 
      type: 'Form', 
      version: "2024", 
      date: "2024-02-01", 
      fileSize: "450 KB",
      description: isAm ? "ለንብረት ጥበቃ አገልግሎት ፈቃድ ለማግኘት የሚያገለግል ቅጽ" : "Form used to obtain a license for asset guarding services."
    },
    { 
      id: 4, 
      ...
      date: "2024-03-10", 
      fileSize: "5.2 MB",
      description: isAm ? "ለአደጋ ጊዜ ሁኔታዎች ዝግጅት እና ምላሽ አሰጣጥ መመሪያ" : "Guidelines for preparation and response to emergency situations."
    },
  ]);
  ...
  const [previewingDoc, setPreviewingDoc] = useState<PublicDocument | null>(null);
  const [newDoc, setNewDoc] = useState({ title: '', type: 'Regulation' as const, description: '' });
  ...
  const t = {
    title: isAm ? "የሰነዶች እና መመሪያዎች አስተዳደር" : "Documents & Regulations Management",
  ...
  const handleDelete = (id: number) => {
    ...
    if (confirm(isAm ? "ይህንን ሰነድ በእርግጠኝነት መሰረዝ ይፈልጋሉ?" : "Are you sure you want to delete this document?")) {
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-1">
            {isAm ? "የህዝብ ሰነዶችን እና የህግ መመሪያዎችን ያቀናብሩ" : "Control public access documents and legal regulations"}
          </p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: <Gavel />, label: isAm ? "ደንቦች" : "Regulations", count: documents.filter(d => d.type === 'Regulation').length, color: "bg-blue-50 text-blue-600" },
          { icon: <Shield />, label: isAm ? "ፖሊሲዎች" : "Policies", count: documents.filter(d => d.type === 'Policy').length, color: "bg-green-50 text-green-600" },
          { icon: <FileText />, label: isAm ? "ቅጾች" : "Forms", count: documents.filter(d => d.type === 'Form').length, color: "bg-amber-50 text-amber-600" },
          { icon: <Download />, label: isAm ? "ጠቅላላ ሰነዶች" : "Total Documents", count: documents.length, color: "bg-purple-50 text-purple-600" },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-3xl border border-gray-100 flex items-center space-x-4 hover:shadow-lg transition-shadow">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${stat.color}`}>
              {stat.icon}
            </div>
            <div>
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">{stat.label}</p>
              <p className="text-xl font-black text-primary">{stat.count} {isAm ? "ፋይሎች" : "Files"}</p>
            </div>
          </div>
        ))}
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">{t.docName}</th>
                <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">{t.type}</th>
                <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">{t.version}</th>
                <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-widest text-gray-400">{t.date}</th>
                <th className="px-8 py-6 text-center text-[10px] font-black uppercase tracking-widest text-gray-400">{t.actions}</th>
              </tr>
            </thead>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-bold uppercase">{doc.fileSize}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest ${
                      doc.type === 'Regulation' ? 'bg-red-50 text-red-600 border border-red-100' : 
                      doc.type === 'Policy' ? 'bg-green-50 text-green-600 border border-green-100' :
                      doc.type === 'Form' ? 'bg-amber-50 text-amber-600 border border-amber-100' :
                      'bg-purple-50 text-purple-600 border border-purple-100'
                    }`}>
                      {isAm ? (
                        doc.type === 'Regulation' ? 'መመሪያ' : 
                        doc.type === 'Policy' ? 'ፖሊሲ' :
                        doc.type === 'Form' ? 'ቅጽ' : 'ማኑዋል'
                      ) : doc.type}
                    </span>
                  </td>
                  <td className="px-8 py-6 text-[10px] font-black text-primary/60">{doc.version}</td>
                  <td className="px-8 py-6 text-[10px] font-black text-gray-400">{doc.date}</td>
                  <td className="px-8 py-6">
                  ...
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <div className="max-w-xs mx-auto space-y-4">
                      ...
                      <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">
                        {isAm ? "ምንም ሰነድ አልተገኘም" : "No documents match your search"}
                      </p>
                    </div>
                  </td>
                </tr>
          ...
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between shrink-0">
                <div>
                  <h2 className="text-xl font-black text-primary tracking-tight">{t.upload}</h2>
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">Add to public repository</p>
                </div>
                <button onClick={() => setIsUploading(false)} className="p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all text-gray-400">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-8 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">{t.docName}</label>
                  <input 
                    type="text" 
                    value={newDoc.title}
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">{t.type}</label>
                  <select 
                    value={newDoc.type}
                  ...
                  >
                    <option value="Regulation">{isAm ? "መመሪያ" : "Regulation"}</option>
                ...
                <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">{t.description}</label>
                  <textarea 
                    ...
                    placeholder={isAm ? "መግለጫ እዚህ ያስገቡ..." : "Enter document summary..."}
                  />
                </div>
                <div className="w-full py-8 bg-gray-50 rounded-[30px] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 hover:bg-white hover:border-secondary transition-all cursor-pointer group">
                   ...
                   <span className="text-[9px] font-black uppercase tracking-widest">{isAm ? "ፋይል ለመምረጥ እዚህ ይጫኑ" : "Select or Drop File"}</span>
                </div>
              </div>
              <div className="p-8 bg-gray-50 border-t border-gray-100 flex justify-end items-center space-x-3 shrink-0">
                <button onClick={() => setIsUploading(false)} className="px-6 py-3 text-gray-400 font-black uppercase text-[10px] tracking-widest hover:text-primary transition-colors">{t.cancel}</button>
                <button onClick={handleUpload} className="blue-gradient text-white px-8 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-[0.1em] shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all">
                  {t.confirm}
                </button>
              </div>
            </motion.div>
        ...
        {editingDoc && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between shrink-0">
                <div>
                  <h2 className="text-xl font-black text-primary tracking-tight">{t.edit}</h2>
                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">Modify document properties</p>
                </div>
                <button onClick={() => setEditingDoc(null)} className="p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all text-gray-400">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-8 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest px-1">{t.version}</label>
                    <input 
                      type="text" 
              ...
              <div className="p-8 bg-gray-50 border-t border-gray-100 flex justify-end items-center space-x-3 shrink-0">
                <button onClick={() => setEditingDoc(null)} className="px-6 py-3 text-gray-400 font-black uppercase text-[10px] tracking-widest hover:text-primary transition-colors">{t.cancel}</button>
                <button onClick={handleEdit} className="bg-amber-500 text-white px-10 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-amber-500/20 hover:scale-105 active:scale-95 transition-all flex items-center space-x-2">
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-blue-50/50 shrink-0">
                <div className="flex items-center space-x-6">
                   ...
                   <div>
                      <h2 className="text-xl font-black text-primary tracking-tight leading-tight line-clamp-1">{previewingDoc.title}</h2>
                      <div className="flex items-center space-x-3 mt-1">
                        <span className="text-[9px] font-black uppercase tracking-widest text-blue-600 bg-blue-100/50 px-3 py-1 rounded-full">{previewingDoc.type}</span>
                        <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">{previewingDoc.version}</span>
                      </div>
                   </div>
                </div>
                <button onClick={() => setPreviewingDoc(null)} className="p-4 bg-white rounded-2xl hover:bg-gray-100 text-gray-400 hover:text-red-500 transition-all shadow-sm">
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm text-center">
                      <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">{t.date}</p>
                      <p className="text-xs font-black text-primary">{previewingDoc.date}</p>
                    </div>
                    <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm text-center">
                      <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Size</p>
                      <p className="text-xs font-black text-primary">{previewingDoc.fileSize}</p>
                    </div>
                    <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm text-center">
                  ...
                  <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 p-10 min-h-[500px] flex flex-col items-center">
                    <div className="w-full flex justify-between items-start mb-12 border-b-2 border-gray-50 pb-8">
                      ...
                      <div className="text-right">
                        <p className="text-[10px] font-black text-primary/20 uppercase tracking-[0.2em]">Federal Police Commission</p>
                        <p className="text-[8px] font-bold text-primary/10 uppercase tracking-widest">Official Document Viewer</p>
                      </div>
                    </div>
                    ...
                    <div className="w-full space-y-6">
                      ...
                      <div className="py-8 px-10 bg-gray-50/30 rounded-3xl border border-gray-50">
                        <p className="text-xs font-bold text-primary/40 leading-relaxed italic text-center">
                          {previewingDoc.description || (isAm ? "ምንም መግለጫ አልተገኘም" : "No description available for this document.")}
                        </p>
                      </div>

                      <div className="grid grid-cols-2 gap-4 pt-12">
                         <div className="h-20 bg-gray-50 overflow-hidden rounded-2xl relative">
                            <div className="absolute inset-0 flex items-center justify-center text-[8px] font-black text-gray-300 uppercase tracking-widest rotate-6">Official Seal</div>
                         </div>
                         <div className="h-20 bg-gray-50 overflow-hidden rounded-2xl relative">
                            <div className="absolute inset-0 flex items-center justify-center text-[8px] font-black text-gray-300 uppercase tracking-widest -rotate-6">Verified Signature</div>
                         </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-amber-50 rounded-[30px] border border-amber-100 flex items-start space-x-4">
                     ...
                     <div>
                        <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1">Disclaimer</p>
                        <p className="text-[10px] font-bold text-amber-900 leading-relaxed">
                          {isAm ? "ያልተፈቀደ ስርጭት የተከለከለ ነው።" : "Unauthorized distribution is prohibited."}
                        </p>
                     </div>
                  </div>
                </div>
              ...
              <div className="p-8 bg-white border-t border-gray-50 flex justify-between items-center shrink-0">
                ...
                <button 
                   ...
                   onClick={() => alert(isAm ? "ሰነዱ በመውረድ ላይ ነው..." : "Downloading...")}
                >
