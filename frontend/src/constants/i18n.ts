export const translations = {
  en: {
    nav: {
      home: "Home",
      services: "Services",
      requirements: "Requirements",
      faq: "FAQ",
      contact: "Contact",
      login: "Login",
      signUp: "Sign Up",
      dashboard: "Dashboard",
    },
    hero: {
      badge: "Official Federal Police Portal",
      titleLine1: "FEDERAL POLICE PRIVATE SECURITY",
      titleLine2: "ORGANIZATION LICENSING",
      federalPolice: "ETHIOPIAN FEDERAL POLICE",
      subtitle:
        "Ensuring safety and security through professional licensing and management.",
      applyNow: "Start Application Now",
      trackStatus: "Track Status",
      login: "Login",
      card: {
        commandCenter: "Federal Command Center",
        securityOversight: "National Security Oversight",
        realTimeLicensing: "Real-time Licensing",
        complianceGrid: "Automated Compliance Grid",
        clearanceActive: "Security Clearance Active",
      },
      stats: {
        agencies: "Registered organization",
        personnel: "Security Personnel",
        licenses: "Active Licenses",
        regions: "Regions Covered",
        agenciesVal: "450+",
        personnelVal: "25,000+",
        licensesVal: "380+",
        regionsVal: "11",
      },
    },
    services: {
      badge: "Our Services",
      title: "Comprehensive Security Management",
      subtitle: "Our Core Services",
      whatWeOffer: "What We Offer",
      newsTitle: "Latest News and Updates",
      newsArchive: "View Archive",
      federalDesk: "Federal Police Desk",
      officialBulletin: "Official Bulletin",
      importantInfo: "Important Information",
      back: "Back",
      closeArticle: "Close Article",
      inquireMore: "Inquire More",
      question: "Have any questions?",
      learnMore: "Learn More",
      getStarted: "Get Started",
      items: [
        {
          title: "New License Application",
          desc: "Digital application process for establishing a new private security organization.",
          features: [
            "Online Document Submission",
            "Fayda ID Verification",
            "Digital Fee Payment",
          ],
        },
        {
          title: "License Renewal",
          desc: "Application to renew existing operational licenses every two years.",
          features: [
            "Compliance Check",
            "Personnel Update",
            "Automatic Notifications",
          ],
        },
        {
          title: "Public News & Announcements",
          desc: "Stay updated with latest federal security bulletins and official news.",
          features: ["Real-time Alerts", "Official Statements", "News Archive"],
        },
        {
          title: "Training Center Accreditation",
          desc: "Certification and licensing for private security training institutions.",
          features: [
            "Curriculum Review",
            "Instructor Vetting",
            "Certification Oversight",
          ],
        },
        {
          title: "Digital Payment",
          desc: "Secure payments via TeleBirr and major Ethiopian banks.",
          features: ["TeleBirr Integration", "CBE Birr", "Bank Transfers"],
        },
        {
          title: "Consultancy",
          desc: "Expert guidance on security regulations and compliance.",
          features: [
            "Regulatory Advice",
            "Security Audits",
            "Compliance Support",
          ],
        },
      ],
    },
    fayda: {
      badge: "Mandatory Requirement",
      title: "Fayda ID Integration",
      desc: "As per the new federal regulations, all private security organization managers and personnel must have a verified Fayda ID (National ID) to apply for or renew licenses.",
      features: [
        "Verified Identity",
        "Biometric Security",
        "Centralized Database",
        "Fraud Prevention",
      ],
      btn: "View All Requirements",
    },
    rules: {
      title: "Public Documents, Rules & Regulations",
      items: [
        { title: "Private Security Proclamation", size: "2.4 MB" },
        { title: "Operational Standards 2024", size: "1.8 MB" },
        { title: "Personnel Vetting Code", size: "1.1 MB" },
        { title: "Organization Licensing Directive", size: "3.2 MB" },
      ],
      learnMore: "Learn More About Compliance",
      pdfFormat: "PDF FORMAT",
      download: "Download",
    },
    trust: {
      badge: "Security Excellence",
      alpha: "Security Protocol Alpha",
      alphaDesc:
        "Orchestrating encrypted verification layers across the national compliance ecosystem.",
      title: "Unified Security, Global Standards",
      desc: "Forging a future where every private security officer is a pillar of national peace through elite digital oversight.",
      networkTitle: "National Network",
      networkDesc:
        "Distributed data synchronization across all 11 regional commands.",
      digitalTitle: "Digital Core",
      digitalDesc:
        "Autonomous verification engines reducing processing time by 80%.",
    },
    faq: {
      title: "Common Questions",
      subtitle: "Frequently Asked Questions",
      searchPlaceholder: "Search for answers...",
      stillQuestions: "Still have questions?",
      stillQuestionsSub:
        "If you couldn't find the answer you were looking for, please contact our support team for further assistance.",
      contactSupport: "Contact Support",
      visitOffice: "Visit Office",
      categories: {
        general: "General",
        licensing: "Licensing",
        technical: "Technical",
        payment: "Payment",
      },
      items: [
        {
          q: "How long does the licensing process take?",
          a: "The standard processing time is 15-30 working days after all documents are verified and the physical bureau inspection is completed.",
          cat: "licensing",
        },
        {
          q: "Why is Fayda ID required?",
          a: "Fayda ID helps verify identities digitally and reduce security risks through biometric integration.",
          cat: "general",
        },
        {
          q: "How often should I renew the license?",
          a: "Private security Organization licenses must be renewed every two years.",
          cat: "licensing",
        },
        {
          q: "Is it mandatory to show uniform samples?",
          a: "Yes, Organization uniforms must be pre-approved to ensure they do not resemble Federal Police uniforms.",
          cat: "licensing",
        },
        {
          q: "What payment methods are accepted?",
          a: "We accept TeleBirr, CBE Birr, and direct bank transfers from all major commercial banks.",
          cat: "payment",
        },
        {
          q: "What should I do if I forget my password?",
          a: "You can use the 'Forgot Password' link on the login page to reset it via your registered email or phone number.",
          cat: "technical",
        },
        {
          q: "Can I track my application status online?",
          a: "Yes, you can track your application status in real-time through your organization dashboard.",
          cat: "technical",
        },
      ],
    },
    contact: {
      badge: "Get In Touch",
      title: "Contact Our Team",
      info: {
        phone: {
          title: "Call Us",
          sub: "Mon-Fri, 8:30 AM - 5:30 PM",
          val: "0115540528",
        },
        email: {
          title: "Email Us",
          sub: "We'll respond within 24 hours",
          val: "support@federalpolice.gov.et",
        },
        visit: {
          title: "Visit Us",
          detail: "Addis Ababa, Mexico Square",
          sub: "Ethiopian Federal Police Commission HQ",
        },
      },
      form: {
        name: "Full Name",
        email: "Email Address",
        subject: "Subject",
        message: "Message",
        placeholder: {
          name: "Full Name",
          email: "Email Address",
          subject: "How can we help?",
          message: "Your message here...",
        },
        submit: "Send Message",
        success: "Your message was sent successfully.",
        error: "Unable to send your message. Please try again.",
        validation: "Please fill out all fields.",
      },
      tabs: {
        admin: "Federal Police Admin",
        systemAdmin: "Federal Police System Admin",
      },
      map: {
        title: "Ethiopian Federal Police HQ",
        desc: "Addis Ababa, Mexico Square. Visit us for physical document verification and inquiries.",
        btn: "Get Directions",
      },
    },
    footer: {
      desc: "Ensuring national security through professional oversight and licensing of private security Organization in Ethiopia.",
      quickLinks: "Quick Links",
      legal: "Legal",
      privacy: "Privacy Policy",
      terms: "Terms of Service",
      copyright: "© 2026 Ethiopian Federal Police. All rights reserved.",
    },
    requirements: {
      title: "General Requirements",
      subtitle: "Licensing Requirements for Private Security Organization",
      mandatory: "Mandatory",
      notice: "Important Notice",
      startApp: "Start Application Now",
      sections: [
        {
          title: "Organization Requirements",
          items: [
            "Trade name designation",
            "Labor and Skill Bureau registration",
            "Trade pre-registration",
            "Memorandum of association",
            "Renewed Trade license",
            "TIN number",
            "Issuance",
            "Organizational structure",
            "Taxpayer Clearance",
            "Internal regulations",
            "Lists of technologies used",
            "Capital according to its level (Bank statement)",
            "Number of offices",
            "Store house",
            "Number of computers",
            "Number of vehicles",
            "Notarized vehicles rent contract or ownership documents",
            "Notarized house rent contract (at least 1 year) or ownership documents",
            "Uniform sample / clothing appearance (photo)",
            "Employee ID card sample (front & back photo)",
            "Employment form (photo)",
            "Employment warranty form (photo)",
            "Logo of organization (photo)",
            "Training status (Manual, Address, Days, Body, Certificate)",
          ],
        },
        {
          title: "Manager, Operation and Administrative head requirements",
          items: [
            "Full name, Gender, Citizenship",
            "Phone Number & Email",
            "Address (Region, zone, woreda, kebele, special location, house number)",
            "Fingerprint from police",
            "Medical result",
            "Training certificate",
            "Support letter from kebele",
            "Proof of Guarantee",
            "Work experience (Minimum 2 years or police or defense or other for 15 years)",
            "Dismissal / resignation record",
            "Educational certificate (Minimum degree)",
            "Fayda / National ID (Mandatory)",
            "Renewed Kebele ID or Passport (One of them mandatory)",
            "Organization identification",
          ],
        },
      ],
      notes: [
        "All documents must be in PDF format.",
        "Ensure your Fayda ID number is valid and verified.",
        "Physical office inspection will be conducted after document verification.",
        "License fees are payable only after application approval.",
      ],
    },
    register: {
      title: "User Registration",
      subtitle: "Please provide your details to create an account",
      success: "Registration Successful!",
      successSub:
        "Your account has been created. You will be redirected to the login page shortly.",
      joinNetwork: "Join the Federal Network",
      joinNetworkSub:
        "Create your account to start the licensing process and manage your personnel through our unified platform.",
      firstName: "First Name",
      firstNamePlaceholder: "First Name",
      middleName: "Middle Name",
      middleNamePlaceholder: "Middle Name",
      lastName: "Last Name",
      lastNamePlaceholder: "Last Name",
      email: "Email Address",
      emailPlaceholder: "email@example.com",
      phone: "Phone Number",
      phonePlaceholder: "+251 9XX XXX XXX",
      username: "Username",
      usernamePlaceholder: "Enter username",
      faydaId: "User Fayda ID",
      faydaPlaceholder: "16-digit Fayda ID",
      verifyFayda: "Verify Fayda ID",
      sendOtp: "Send SMS verification",
      otpPlaceholder: "6-digit SMS code",
      password: "Password",
      confirmPassword: "Confirm Password",
      submit: "Register Now",
      submitting: "Creating Account...",
      alreadyAccount: "Already have an account?",
      features: [
        {
          title: "Official Licensing",
          desc: "Get your federal police approved license digitally.",
        },
        {
          title: "Personnel Tracking",
          desc: "Manage and verify your staff with Fayda ID.",
        },
        {
          title: "Real-time Updates",
          desc: "Track your application status at every step.",
        },
      ],
    },
    auth: {
      userLogin: "User Login",
      adminLogin: "Portal Login",
      superAdminLogin: "Super Admin Access",
      faydaId: "Fayda ID",
      username: "Username",
      password: "Password",
      signIn: "Sign In",
      noAccount: "Don't have an account?",
      register: "Register Now",
      otpOption: "Verify identity using OTP",
      otpMethodEmail: "Email",
      otpMethodSms: "SMS",
      sendOtp: "Send Verification Code",
      otpPlaceholder: "Enter 6-digit code",
      otpError: "Invalid verification code",
      secureAccessPortal: "Secure Access Portal",
      forgotPassword: "Forgot Password?",
    },
    newApplication: {
      formalLetterTitle: "Formal letter approval required",
      formalLetterMessage:
        "Your formal letter must be approved for {year} before you can open the new application page.",
      returnToDashboard: "Return to Dashboard",
    },
    dashboard: {
      welcome: "Welcome back",
      overview: "Overview",
      formalLetter: "Formal Letter",
      newApplication: "New Application",
      renewal: "Renewal",
      trackStatus: "Track Status",
      payment: "Payment",
      agreement: "Agreement",
      license: "My License",
      licenseManagement: "License Management",
      reports: "Reports",
      hrmsReports: "HRMS Reports",
      inspections: "Manage Inspections",
      gps: "GPS Tracking",
      agencies: "Organizations Management",
      applications: "Applications Review",
      userManagement: "User Management",
      backups: "Backup & Recovery",
      permissions: "Permissions Management",
      settings: "Settings",
      agencyDashboard: "Agency Dashboard",
      adminDashboard: "Federal Police Admin",
      superAdminDashboard: "Super Admin Command Center",
    },
    reports: {
      title: "Federal Police Admin Reports",
      subtitle: "Comprehensive federal police administrative reports.",
      last30Days: "Last 30 Days",
      generatePdf: "Generate PDF",
      weekly: "Weekly",
      monthly: "Monthly",
      yearly: "Yearly",
      revenueGrowth: "Revenue Growth",
      appVolume: "Application Volume",
      activeAgencies: "Active Organizations",
      pendingRenewals: "Pending Renewals",
      totalPersonnel: "Total Personnel",
      thisMonth: "this month",
      dueIn7Days: "Due in 7 days",
      thisWeek: "+124 this week",
      success: "Report generated successfully!",
      loading: "Generating report...",
      applications: "Applications",
      licenses: "Licenses",
      inspections: "Inspections",
      agreements: "Agreements",
      agencies: "Organizations",
    },
    settings: {
      title: "System Ecosystem Controls",
      subtitle: "Global configuration and underlying security orchestration",
      tabs: {
        general: "General",
        security: "Security",
        notifications: "Notifications",
        database: "Database",
      },
      general: {
        coreId: "Core Identification",
        branding: "Platform Branding",
        authority: "Contact Authority",
        automation: "Licensing Automation",
        fayda: "Fayda ID Enforcement",
        faydaDesc: "Require national ID for all individual background checks.",
        expiry: "Intelligent Expiry Alerts",
        expiryDesc:
          "Automated SMS orchestration 30 days prior to license burnout.",
        audit: "Audit Log Retention",
        auditDesc:
          "Maintain immutable logs for 5 years per federal regulation.",
      },
      security: {
        hardening: "Hardening & Access",
        twoFactor: "Two-Factor Strategy",
        mandatoryAdmin: "Mandatory for all Admins",
        optionalAgency: "Optional for Organizations",
        disabled: "Disabled",
        deviceTrust: "Device Trust Pool",
        managedDevices: "4 Managed Devices",
        revokeAll: "Revoke All",
        policy: "Credential Policy",
        entropy: "Password Entropy",
        secure: "Secure (128-bit)",
        resetTokens: "Reset All API Tokens",
        rotateKeys: "Rotate Encryption Keys",
      },
      notifications: {
        orchestrator: "Notification Orchestrator",
        desc: "Control how the system communicates with organization, applicants, and regional blocks.",
        critical: "Critical System Alerts",
        pushEmail: "Push, Email",
        agencyReminders: "Agency License Reminders",
        sms: "SMS",
        newApps: "New Application Requests",
        dashboardOnly: "Dashboard Only",
        configure: "Configure",
      },
      database: {
        integrity: "Database Integrity",
        cluster: "Cloud Firestore Cluster • High Availability",
        totalDocs: "Total Documents",
        latency: "Avg. Latency",
        sync: "Sync Status",
        maintenance: "Maintenance Operations",
        fullBackup: "Full DB Backup",
        backupDesc: "Schedule a complete snapshot of all datasets.",
        scan: "Inconsistency Scan",
        scanDesc: "Identify orphaned records and link decay.",
      },
      save: "Save Parameters",
      redZone: {
        title: "System Red Zone",
        subtitle: "Destructive operations requiring secondary auth clearance",
        wipeLogs: "Wipe Audit Logs",
        failover: "Initiate Failover Mode",
      },
      alertSuccess:
        "Settings updated successfully! Changes are now live across the system.",
    },
    hrmsReports: {
      title: "Agencies HRMS Reports",
      subtitle: "Reports retrieved from individual Organization HRMS systems",
      search: "Search organization...",
      exportAll: "Export All",
      totalPersonnel: "Total Security Personnel",
      certifiedStaff: "Certified Trained Staff",
      activeDeployments: "Active Deployments",
      table: {
        agency: "Organization Name",
        totalStaff: "Total Staff",
        trained: "Trained",
        deployed: "Deployed",
        lastUpdate: "Last Update",
        actions: "Actions",
      },
      modal: {
        title: "Organization HRMS Reports",
        subtitle: "Management System",
        backToSelection: "Back to selection",
        downloadPdf: "Download PDF",
        generating: "Generating data visualization...",
        viewReport: "View Report",
      },
      reportTypes: {
        weekly: {
          name: "Weekly Performance",
          desc: "Weekly activity summary and key metrics.",
        },
        monthly: {
          name: "Monthly Performance",
          desc: "Statistical overview of organization activities.",
        },
        yearly: {
          name: "Yearly Performance",
          desc: "Annual performance review and trend analysis.",
        },
        personnel: {
          name: "Personnel Documents",
          desc: "Background checks and legal IDs of guards.",
        },
        payroll: {
          name: "Payroll & Benefits",
          desc: "Wages, insurance, and social security compliance.",
        },
        training: {
          name: "Training Certs",
          desc: "Certification status of all security personnel.",
        },
      },
      success: "HRMS Report downloaded successfully!",
    },
    common: {
      submit: "Submit",
      cancel: "Cancel",
      next: "Next",
      back: "Back",
      upload: "Upload",
      download: "Download",
      search: "Search",
      filter: "Filter",
      loading: "Loading...",
      success: "Success",
      error: "Error",
      edit: "Edit",
      delete: "Delete",
      save: "Save",
      add: "Add",
      status: "Status",
      active: "Active",
      inactive: "Inactive",
      suspended: "Suspended",
    },
  },
  am: {
    nav: {
      home: "መነሻ",
      services: "አገልግሎቶች",
      requirements: "መስፈርቶች",
      faq: "ተደጋጋሚ ጥያቄዎች",
      contact: "እውቂያ",
      login: "ግባ",
      signUp: "ይመዝገቡ",
      dashboard: "ዳሽቦርድ",
    },
    hero: {
      badge: "ይፋዊ የፌዴራል ፖሊስ ፖርታል",
      titleLine1: "የፌዴራል ፖሊስ የግል ጥበቃ",
      titleLine2: "የድርጅት ፈቃድ",
      federalPolice: "የኢትዮጵያ ፌዴራል ፖሊስ",
      subtitle: "በሙያዊ ፈቃድ እና አስተዳደር ደህንነትን ማረጋገጥ።",
      applyNow: "ማመልከቻውን አሁኑኑ ይጀምሩ",
      trackStatus: "ሁኔታውን ይከታተሉ",
      login: "ግባ",
      card: {
        commandCenter: "የፌዴራል ዕዝ ማዕከል",
        securityOversight: "ብሄራዊ የደህንነት ቁጥጥር",
        realTimeLicensing: "የእውነተኛ ጊዜ ፈቃድ",
        complianceGrid: "አውቶማቲክ የተገዢነት ስርዓት",
        clearanceActive: "የደህንነት ፍቃድ ንቁ ነው",
      },
      stats: {
        agencies: "የተመዘገቡ ድርጅቶች",
        personnel: "የደህንነት ሰራተኞች",
        licenses: "ንቁ ፈቃዶች",
        regions: "የተሸፈኑ ክልሎች",
        agenciesVal: "450+",
        personnelVal: "25,000+",
        licensesVal: "380+",
        regionsVal: "11",
      },
    },
    services: {
      badge: "አገልግሎቶቻችን",
      title: "አጠቃላይ የደህንነት አስተዳደር",
      subtitle: "ዋና ዋና አገልግሎቶቻችን",
      whatWeOffer: "እኛ የምናቀርበው",
      newsTitle: "የቅርብ ጊዜ ዜናዎች እና መረጃዎች",
      newsArchive: "ሁሉንም ይመልከቱ",
      federalDesk: "የፌዴራል ፖሊስ",
      officialBulletin: "ይፋዊ መግለጫ",
      importantInfo: "ጠቃሚ መረጃ",
      back: "ተመለስ",
      closeArticle: "ጽሁፉን ዝጋ",
      inquireMore: "ተጨማሪ ይጠይቁ",
      question: "ጥያቄ አለዎት?",
      learnMore: "ተጨማሪ ያንብቡ",
      getStarted: "አሁኑኑ ይጀምሩ",
      items: [
        {
          title: "አዲስ የፈቃድ ማመልከቻ",
          desc: "አዲስ የግል የግል ጥበቃ ድርጅት ለመክፈት የሚቀርብ ዲጂታል ማመልከቻ።",
          features: [
            "የመስመር ላይ ሰነድ ማስገባት",
            "የፋይዳ መታወቂያ ማረጋገጫ",
            "ዲጂታል የክፍያ ስርዓት",
          ],
        },
        {
          title: "የፈቃድ እድሳት",
          desc: "ያለውን ፈቃድ በየሁለት ዓመቱ ለማደስ የሚቀርብ ማመልከቻ።",
          features: ["የተገዢነት ማረጋገጫ", "የሰራተኞች መረጃ ማሻሻያ", "አውቶማቲክ ማሳወቂያዎች"],
        },
        {
          title: "የሕዝብ ዜና እና ማስታወቂያዎች",
          desc: "የቅርብ ጊዜ የፌዴራል የደህንነት መግለጫዎችን እና ይፋዊ ዜናዎችን ይከታተሉ።",
          features: ["የእውነተኛ ጊዜ ማንቂያዎች", "ይፋዊ መግለጫዎች", "የዜና መዝገብ"],
        },
        {
          title: "የሥልጠና ማዕከል እውቅና",
          desc: "ለግል የደህንነት ስልጠና ተቋማት የምስክር ወረቀት እና ፈቃድ መስጠት።",
          features: ["የስርአተ ትምህርት ግምገማ", "የአሰልጣኞች ማጣሪያ", "የምስክር ወረቀት ቁጥጥር"],
        },
        {
          title: "ዲጂታል ክፍያ",
          desc: "በቴሌብር እና በዋና ዋና የኢትዮጵያ ባንኮች ደህንነቱ የተጠበቀ ክፍያ ይፈጽሙ።",
          features: ["የቴሌብር ውህደት", "ሲቢኢ ብር", "የባንክ ዝውውር"],
        },
        {
          title: "ምክክር",
          desc: "በደህንነት ደንቦች እና ተገዢነት ላይ የባለሙያ መመሪያ።",
          features: ["የቁጥጥር ምክር", "የደህንነት ኦዲት", "የተገዢነት ድጋፍ"],
        },
      ],
    },
    fayda: {
      badge: "ግዴታ መስፈርት",
      title: "የፋይዳ መታወቂያ ውህደት",
      desc: "በአዲሱ የፌዴራል ደንብ መሠረት ሁሉም የግል የግል ጥበቃ ድርጅት ሥራ አስኪያጆች እና ሠራተኞች ለፈቃድ ለማመልከት ወይም ለማደስ የተረጋገጠ የፋይዳ መታወቂያ (ብሔራዊ መታወቂያ) ሊኖራቸው ይገባል።",
      features: [
        "የተረጋገጠ ማንነት",
        "ባዮሜትሪክ ደህንነት",
        "ማዕከላዊ የውሂብ ጎታ",
        "የማጭበርበር መከላከል",
      ],
      btn: "ሁሉንም መስፈርቶች ይመልከቱ",
    },
    rules: {
      title: "የሕዝብ ሰነዶች፣ ደንቦች እና መመሪያዎች",
      items: [
        { title: "የግል ደህንነት አዋጅ", size: "2.4 MB" },
        { title: "የአሰራር ደረጃዎች 2024", size: "1.8 MB" },
        { title: "የሰራተኞች ምርመራ ኮድ", size: "1.1 MB" },
        { title: "የድርጅት ፈቃድ መመሪያ", size: "3.2 MB" },
      ],
      learnMore: "ስለ ደንቦች የበለጠ ይረዱ",
      pdfFormat: "PDF ፎርማት",
      download: "ያውርዱ",
    },
    trust: {
      badge: "የላቀ ደህንነት",
      alpha: "የደህንነት ፕሮቶኮል አልፋ",
      alphaDesc: "በብሔራዊ የተገዢነት ሥነ-ምህዳር ውስጥ የተመሰጠሩ የማረጋገጫ ንብርብሮችን ማቀናጀት።",
      title: "የጋራ ደህንነት፣ የሃገር ክብር",
      desc: "የፌዴራል ፖሊስ የጥበቃ ድርጅቶችን  ደረጃ በማሳደግ የሃገራችንን ሰላም እና ደህንነት በጋራ  እናረጋግጣለን።",
      networkTitle: "ብሄራዊ ተደራሽነት",
      networkDesc: "በሁሉም ክልሎች የተቀናጀ የክትትል ስርዓት።",
      digitalTitle: "ዲጂታል ፈጠራ",
      digitalDesc: "ዘመናዊ እና ፈጣን የፈቃድ አሰጣጥ ሂደት።",
    },
    faq: {
      title: "የተለመዱ ጥያቄዎች",
      subtitle: "ተደጋጋሚ ጥያቄዎች",
      searchPlaceholder: "መልሶችን ይፈልጉ...",
      stillQuestions: "አሁንም ጥያቄዎች አሉዎት?",
      stillQuestionsSub:
        "የሚፈልጉትን መልስ ካላገኙ፣ እባክዎን ለተጨማሪ እርዳታ የድጋፍ ቡድናችንን ያነጋግሩ።",
      contactSupport: "ድጋፍ ሰጪን ያግኙ",
      visitOffice: "ቢሮ ይጎብኙ",
      categories: {
        general: "አጠቃላይ",
        licensing: "የፈቃድ አሰጣጥ",
        technical: "ቴክኒካዊ",
        payment: "ክፍያ",
      },
      items: [
        {
          q: "የፈቃድ ሂደቱ ምን ያህል ጊዜ ይወስዳል?",
          a: "መደበኛው የሂደት ጊዜ ሁሉም ሰነዶች ከተረጋገጡ እና የአካል ቢሮ ፍተሻ ከተጠናቀቀ በኋላ ከ15-30 የሥራ ቀናት ነው።",
          cat: "licensing",
        },
        {
          q: "የፋይዳ መታወቂያ ለምን አስፈለገ?",
          a: "የፋይዳ መታወቂያ የግለሰቦችን ማንነት በዲጂታል መንገድ ለማረጋገጥ እና የደህንነት ስጋቶችን ለመቀነስ ይረዳል።",
          cat: "general",
        },
        {
          q: "የፈቃድ እድሳት በየስንት ጊዜው ነው?",
          a: "የግል ጥበቃ ድርጅት ፈቃድ በየአንድ ዓመቱ መታደስ አለበት።",
          cat: "licensing",
        },
        {
          q: "የደንብ ልብስ ናሙና ማሳየት ግዴታ ነው?",
          a: "አዎ፣ የድርጅቱ የደንብ ልብስ ከፌዴራል ፖሊስ የደንብ ልብስ ጋር እንዳይመሳሰል በቅድሚያ መጽደቅ አለበት።",
          cat: "licensing",
        },
        {
          q: "ምን ዓይነት የክፍያ ዘዴዎች ተቀባይነት አላቸው?",
          a: "ቴሌብር፣ ሲቢኢ ብር እና ከሁሉም ዋና ዋና የንግድ ባንኮች የቀጥታ የባንክ ዝውውሮችን እንቀበላለን።",
          cat: "payment",
        },
        {
          q: "የይለፍ ቃሌን ብረሳ ምን ማድረግ አለብኝ?",
          a: "በተመዘገቡበት የኢሜል አድራሻ ወይም ስልክ ቁጥር በመጠቀም የይለፍ ቃልዎን ለመቀየር በመግቢያ ገጹ ላይ ያለውን 'የይለፍ ቃል ረስተዋል' የሚለውን ሊንክ መጠቀም ይችላሉ።",
          cat: "technical",
        },
        {
          q: "የማመልከቻ ሁኔታዬን በመስመር ላይ መከታተል እችላለሁ?",
          a: "አዎ፣ የእርስዎን የማመልከቻ ሁኔታ በድርጅት ዳሽቦርድዎ በኩል በቅጽበት መከታተል ይችላሉ።",
          cat: "technical",
        },
      ],
    },
    contact: {
      badge: "ያግኙን",
      title: "ቡድናችንን ያነጋግሩ",
      info: {
        phone: {
          title: "ይደውሉልን",
          sub: "ከሰኞ-አርብ፣ ከጠዋቱ 2:30 - ከሰዓት 11:30",
          val: "0115540528",
        },
        email: {
          title: "ኢሜል ያድርጉልን",
          sub: "በ24 ሰዓታት ውስጥ ምላሽ እንሰጣለን",
          val: "support@federalpolice.gov.et",
        },
        visit: {
          title: "ይጎብኙን",
          detail: "አዲስ አበባ፣ ሜክሲኮ አደባባይ",
          sub: "የኢትዮጵያ ፌዴራል ፖሊስ ኮሚሽን ዋና መሥሪያ ቤት",
        },
      },
      form: {
        name: "ሙሉ ስም",
        email: "የኢሜል አድራሻ",
        subject: "ርዕሰ ጉዳይ",
        message: "መልዕክት",
        placeholder: {
          name: "ሙሉ ስም",
          email: "የኢሜል አድራሻ",
          subject: "እንዴት ልንረዳዎ እንችላለን?",
          message: "መልዕክትዎን እዚህ ይጻፉ...",
        },
        submit: "መልዕክት ላክ",
        success: "መልእክትዎ በተሳካ ሁኔታ ተልኳል።",
        error: "መልእክትዎን ማስተላለፍ አልቻልንም። እባክዎ ደግመው ይሞክሩ።",
        validation: "እባክዎ ሁሉንም ቦታ ይሙሉ።",
      },
      tabs: {
        admin: "የፌዴራል ፖሊስ አስተዳዳሪ",
        systemAdmin: "የፌዴራል ፖሊስ ስርዓት አስተዳዳሪ",
      },
      map: {
        title: "የኢትዮጵያ ፌዴራል ፖሊስ ዋና መሥሪያ ቤት",
        desc: "አዲስ አበባ፣ ሜክሲኮ አደባባይ። ለአካል ሰነድ ማረጋገጫ እና ጥያቄዎች ይጎብኙን።",
        btn: "አቅጣጫዎችን ያግኙ",
      },
    },
    footer: {
      desc: "በኢትዮጵያ ውስጥ የግል ጥበቃ  ድርጅቶች ሙያዊ ቁጥጥር እና የብቃት  ማረጋገጫ ፈቃድ በመስጠት ብሔራዊ ደህንነትን ማረጋገጥ።",
      quickLinks: "ፈጣን አገናኞች",
      legal: "ህጋዊ",
      privacy: "የግላዊነት ፖሊሲ",
      terms: "የአጠቃቀም ውል",
      copyright: "© 2026 የኢትዮጵያ ፌዴራል ፖሊስ። መብቱ በህግ የተጠበቀ ነው።",
    },
    register: {
      title: "የተጠቃሚ ምዝገባ",
      subtitle: "መለያ ለመፍጠር እባክዎን የእርስዎን መረጃ ይስጡ",
      success: "ምዝገባው ተሳክቷል!",
      successSub: "የተጠቃሚ መለያዎ ተፈጥሯል። በቅርቡ ወደ መግቢያ ገጽ ይዛወራሉ።",
      joinNetwork: "የፌዴራል መረቡን ይቀላቀሉ",
      joinNetworkSub:
        "የ ፊቃድ መስጠት ሂደቱን ለመጀመር እና ሰራተኞችዎን በጋራ ፖርታላችን ለማስተዳደር ይመዝገቡ።",
      firstName: "የመጀመሪያ ስም",
      firstNamePlaceholder: "የመጀመሪያ ስም",
      middleName: "የአባት ስም",
      middleNamePlaceholder: "የአባት ስም",
      lastName: "የአያት ስም",
      lastNamePlaceholder: "የአያት ስም",
      email: "የኢሜል አድራሻ",
      emailPlaceholder: "email@example.com",
      phone: "የስልክ ቁጥር",
      phonePlaceholder: "+251 9XX XXX XXX",
      username: "የተጠቃሚ ስም",
      usernamePlaceholder: "የተጠቃሚ ስም ያስገቡ",
      faydaId: "የተጠቃሚ ፋይዳ መታወቂያ",
      faydaPlaceholder: "ባለ 16 አሃዝ ፋይዳ መታወቂያ",
      verifyFayda: "ፋይዳ መታወቂያን ያረጋግጡ",
      sendOtp: "የSMS ማረጋገጫ ይላኩ",
      otpPlaceholder: "ባለ 6 አሃዝ SMS ኮድ",
      password: "የይለፍ ቃል",
      confirmPassword: "የይለፍ ቃል ያረጋግጡ",
      submit: "አሁኑኑ ይመዝገቡ",
      submitting: "መለያ በመፍጠር ላይ...",
      alreadyAccount: "ቀድሞውኑ መለያ አለዎት?",
      features: [
        { title: "ይፋዊ ፈቃድ", desc: "በፌዴራል ፖሊስ የጸደቀ ፈቃድዎን በዲጂታል መንገድ ያግኙ።" },
        { title: "የሰራተኞች ክትትል", desc: "ሰራተኞችዎን በፋይዳ መታወቂያ ያስተዳድሩ እና ያረጋግጡ።" },
        { title: "የእውነተኛ ጊዜ ዝመናዎች", desc: "የማመልከቻዎን ሁኔታ በየደረጃው ይከታተሉ።" },
      ],
    },
    requirements: {
      title: "አጠቃላይ መስፈርቶች",
      subtitle: "የግል የግል ጥበቃ ድርጅት ለመክፈት የሚያስፈልጉ መስፈርቶች",
      mandatory: "ግዴታ",
      notice: "አስፈላጊ ማሳሰቢያ",
      noticeSub: "እባክዎን ከማመልከትዎ በፊት እነዚህን ነጥቦች ያስተውሉ",
      startApp: "አሁን ማመልከቻ ይጀምሩ",
      sections: [
        {
          title: "የድርጅት መስፈርቶች",
          items: [
            "የንግድ ስም ስያሜ",
            "የሥራ እና ክህሎት ቢሮ ምዝገባ",
            "የንግድ ቅድመ-ምዝገባ",
            "የመመስረቻ ጽሑፍ",
            "የኢንሹራንስ ሽፋን ",
            "የግብር ከፋይ መለያ ቁጥር (TIN)",
            "የግብር ከፋይነት  ክሊራንስ ",
            "የድርጅታዊ መዋቅር",
            "የታደሰ የንግድ ፈቃድ",
            "የውሥጥ መተዳደሪያ ደንብ",
            "ጥቅም ላይ የሚውሉ ቴክኖሎጂዎች ዝርዝር",
            "እንደ ደረጃው ካፒታል (የባንክ መግለጫ)",
            "የቢሮዎች ብዛት",
            "ግምጃ ቤት ",
            "የኮምፒውተሮች ብዛት",
            "የተሽከርካሪዎች ብዛት",
            "በውል የተረጋገጠ የተሽከርካሪ ኪራይ ውል ወይም የባለቤትነት ሰነድ",
            "ቢያንስ አንድ ዓመት የቀረው በውል የተረጋገጠ የቤት ኪራይ ውል ወይም የቤት ባለቤትነት ሰነድ",
            "የደንብ ልብስ ናሙና (በፎቶ)",
            "የድርጅት የሰራተኛ መታወቂያ ናሙና (ፊት እና ጀርባ በፎቶ)",
            "የቅጥር ቅጽ (በፎቶ)",
            "የቅጥር ዋስትና ቅጽ (በፎቶ)",
            "የድርጅቱ አርማ (በፎቶ)",
            "የስልጠና ሁኔታ (ማኑዋል፣ አድራሻ፣ ቀናት፣ አካል፣ ሰርተፍኬት)",
          ],
        },
        {
          title: "የሥራ  አስኪያጅ፣የአፕሬሽን፣የአስተዳደርና ፋይናንስ ኃላፊዎች  መስፈርት",
          items: [
            "ሙሉ ስም፣ ጾታ፣ ዜግነት",
            "የስልክ ቁጥር እና ኢሜል",
            "አድራሻ (ክልል፣ ዞን፣ ወረዳ፣ ቀበሌ፣ ልዩ ቦታ፣ የቤት ቁጥር)",
            "የጣት አሻራ ከፖሊስ",
            "የሕክምና ውጤት",
            "የስልጠና ምስክር ወረቀት",
            "የድጋፍ ደብዳቤ ከቀበሌ",
            "የዋስትና ማስረጃ",
            "የሥራ ልምድ (ቢያንስ 2 ዓመት ወይም በፖሊስ ወይም በመከላከያ ወይም በሌላ ለ15 ዓመታት የሠሩ)",
            "ከሥራ የመልቀቂያ ማስረጃ",
            "የትምህርት ማስረጃ (ቢያንስ ዲግሪ)",
            "ፋይዳ / ብሔራዊ መታወቂያ (ግዴታ)",
            "የታደሰ የቀበሌ መታወቂያ ወይም ፓስፖርት (ከሁለቱ አንዱ ግዴታ)",
            "የድርጅት መታወቂያ",
          ],
        },
      ],
      notes: [
        "ሁሉም ሰነዶች በፒዲኤፍ (PDF) ቅርጸት መሆን አለባቸው።",
        "የፋይዳ መታወቂያ ቁጥርዎ ትክክለኛ መሆኑን ያረጋግጡ።",
        "የቢሮ ፍተሻ የሚከናወነው ሰነዶች ከተረጋገጡ በኋላ ነው።",
        "የፈቃድ ክፍያ የሚፈጸመው ማመልከቻው ተቀባይነት ካገኘ በኋላ ነው።",
      ],
    },
    auth: {
      userLogin: "የተጠቃሚ መግቢያ",
      adminLogin: "የፖርታል መግቢያ",
      superAdminLogin: "የዋና አስተዳዳሪ መግቢያ",
      faydaId: "ፋይዳ መታወቂያ",
      username: "ተጠቃሚ ስም",
      password: "የይለፍ ቃል",
      signIn: "ግባ",
      noAccount: "መለያ የለዎትም?",
      register: "አሁኑኑ ይመዝገቡ",
      otpOption: "በOTP ማንነትዎን ያረጋግጡ",
      otpMethodEmail: "ኢሜል",
      otpMethodSms: "ኤስኤምኤስ (SMS)",
      sendOtp: "የማረጋገጫ ኮድ ይላኩ",
      otpPlaceholder: "ባለ 6 አሃዝ ኮድ ያስገቡ",
      otpError: "ትክክለኛ ያልሆነ የማረጋገጫ ኮድ",
      secureAccessPortal: "ደህንነቱ የተጠበቀ መዳረሻ ፖርታል",
      forgotPassword: "የይለፍ ቃል ረሱ?",
    },
    newApplication: {
      formalLetterTitle: "የመደበኛ ደብዳቤ ማረጋገጫ ያስፈልጋል",
      formalLetterMessage:
        "አዲሱን የማመልከቻ ገጽ ለመክፈት የ{year} የመደበኛ ደብዳቤዎ መጽደቅ ያስፈልጋል።",
      returnToDashboard: "ወደ ዳሽቦርድ ይመለሱ",
    },
    dashboard: {
      welcome: "እንኳን ደህና መጡ",
      overview: "አጠቃላይ እይታ",
      formalLetter: "የይፋዊ ደብዳቤ",
      newApplication: "አዲስ ማመልከቻ",
      renewal: "እድሳት",
      trackStatus: "ሁኔታውን ይከታተሉ",
      payment: "ክፍያ",
      agreement: "ስምምነት",
      license: "የእኔ ፈቃድ",
      licenseManagement: "የፈቃድ አስተዳደር",
      reports: "ሪፖርቶች",
      hrmsReports: "የHRMS ሪፖርቶች",
      inspections: "ምርመራ አስተዳደር",
      gps: "የጂፒኤስ ክትትል",
      agencies: "የድርጅቶች አስተዳደር",
      applications: "የማመልከቻዎች ግምገማ",
      userManagement: "የተጠቃሚዎች አስተዳደር",
      backups: "መጠባበቂያ እና ማግኛ",
      permissions: "ፍቃዶች እና ሚናዎች",
      settings: "ቅንብሮች",
      agencyDashboard: "የድርጅት ዳሽቦርድ",
      adminDashboard: "የፌዴራል ፖሊስ አስተዳዳሪ",
      superAdminDashboard: "የዋና አስተዳዳሪ የትዕዛዝ ማዕከል",
    },
    reports: {
      title: "የፌዴራል ፖሊስ አስተዳደር ሪፖርቶች",
      subtitle: "አጠቃላይ የፌዴራል ፖሊስ አስተዳደር ሪፖርቶች።",
      last30Days: "ያለፉት 30 ቀናት",
      generatePdf: "PDF ፍጠር",
      weekly: "ሳምንታዊ",
      monthly: "ወርሃዊ",
      yearly: "ዓመታዊ",
      revenueGrowth: "የገቢ ዕድገት",
      appVolume: "የማመልከቻ መጠን",
      activeAgencies: "ንቁ ድርጅቶች",
      pendingRenewals: "የሚጠባበቁ እድሳቶች",
      totalPersonnel: "ጠቅላላ ሰራተኞች",
      thisMonth: "በዚህ ወር",
      dueIn7Days: "በ7 ቀናት ውስጥ የሚደርስ",
      thisWeek: "በዚህ ሳምንት",
      success: "ሪፖርቱ በተሳካ ሁኔታ ተፈጥሯል!",
      loading: "ሪፖርት በመፍጠር ላይ...",
      applications: "ማመልከቻዎች",
      licenses: "ፈቃዶች",
      inspections: "ምርመራዎች",
      agreements: "ስምምነቶች",
      agencies: "ድርጅቶች",
    },
    settings: {
      title: "የስርዓት ስነ-ምህዳር መቆጣጠሪያዎች",
      subtitle: "አጠቃላይ ውቅር እና ዋና የደህንነት ቅንብሮች",
      tabs: {
        general: "አጠቃላይ",
        security: "ደህንነት",
        notifications: "ማሳወቂያዎች",
        database: "የመረጃ ቋት",
      },
      general: {
        coreId: "ዋና መለያ",
        branding: "የፕላትፎርም ብራንዲንግ",
        authority: "የእውቂያ ባለስልጣን",
        automation: "የፈቃድ አሰጣጥ አውቶሜሽን",
        fayda: "የፋይዳ መታወቂያ ማስገደድ",
        faydaDesc: "ለሁሉም የግል ታሪክ ፍተሻዎች ብሔራዊ መታወቂያ ያስፈልጋል።",
        expiry: "ብልህ የማብቂያ ማንቂያዎች",
        expiryDesc: "ፈቃድ ከማለቁ 30 ቀናት በፊት አውቶማቲክ የኤስኤምኤስ ማሳሰቢያ።",
        audit: "የኦዲት ሎግ ማቆያ",
        auditDesc: "በፌዴራል ደንብ መሠረት የማይለወጡ ሎጎችን ለ5 ዓመታት ያቆዩ።",
      },
      security: {
        hardening: "ጥንካሬ እና መዳረሻ",
        twoFactor: "ባለ ሁለት ደረጃ ማረጋገጫ ስትራቴጂ",
        mandatoryAdmin: "ለሁሉም አስተዳዳሪዎች ግዴታ",
        optionalAgency: "ለድርጅቶች አማራጭ",
        disabled: "ቦዝኗል",
        deviceTrust: "የታመኑ መሳሪያዎች",
        managedDevices: "4 የሚተዳደሩ መሳሪያዎች",
        revokeAll: "ሁሉንም ሰርዝ",
        policy: "የማረጋገጫ ፖሊሲ",
        entropy: "የይለፍ ቃል ጥንካሬ",
        secure: "ደህንነቱ የተጠበቀ (128-ቢት)",
        resetTokens: "ሁሉንም የAPI ቶከኖች አስጀምር",
        rotateKeys: "የምስጠራ ቁልፎችን አሽከርክር",
      },
      notifications: {
        orchestrator: "የማሳወቂያ አስተባባሪ",
        desc: "ስርዓቱ ከድርጅቶች፣ አመልካቾች እና የክልል ብሎኮች ጋር እንዴት እንደሚገናኝ ይቆጣጠሩ።",
        critical: "ወሳኝ የስርዓት ማንቂያዎች",
        pushEmail: "ፑሽ፣ ኢሜል",
        agencyReminders: "የድርጅት ፈቃድ ማሳሰቢያዎች",
        sms: "ኤስኤምኤስ (SMS)",
        newApps: "አዲስ የማመልከቻ ጥያቄዎች",
        dashboardOnly: "በዳሽቦርድ ብቻ",
        configure: "አዋቅር",
      },
      database: {
        integrity: "የመረጃ ቋት ትክክለኛነት",
        cluster: "የክላውድ ፋየር ስቶር ክላስተር • ከፍተኛ ተገኝነት",
        totalDocs: "ጠቅላላ ሰነዶች",
        latency: "አማካኝ መዘግየት",
        sync: "የማመሳሰል ሁኔታ",
        maintenance: "የጥገና ስራዎች",
        fullBackup: "የመረጃ ቋት መጠባበቂያ",
        backupDesc: "የሁሉንም የውሂብ ስብስቦች ሙሉ ቅጽበታዊ ገጽ እይታ ያቅዱ።",
        scan: "የአለመጣጣም ፍተሻ",
        scanDesc: "የተሳሳቱ መዝገቦችን እና የተበላሹ አገናኞችን ይለዩ።",
      },
      save: "ቅንብሮቹን አስቀምጥ",
      redZone: {
        title: "የስርዓት ቀይ ዞን",
        subtitle: "ሁለተኛ ደረጃ የማረጋገጫ ፍቃድ የሚያስፈልጋቸው አውዳሚ ስራዎች",
        wipeLogs: "የኦዲት ሎጎችን አጽዳ",
        failover: "የFailover ሁነታን ጀምር",
      },
      alertSuccess: "ቅንብሮች በተሳካ ሁኔታ ተዘምነዋል! ለውጦቹ አሁን በመላው ስርዓቱ ላይ ተግባራዊ ሆነዋል።",
    },
    hrmsReports: {
      title: "የድርጅቶች HRMS ሪፖርቶች",
      subtitle: "ከድርጅቶች የሰው ኃይል አስተዳደር ስርዓት የተገኙ ሪፖርቶች",
      search: "ድርጅት ይፈልጉ...",
      exportAll: "ሁሉንም አውርድ",
      totalPersonnel: "ጠቅላላ የደህንነት ሰራተኞች",
      certifiedStaff: "የሰለጠኑ ሰራተኞች",
      activeDeployments: "ንቁ ስምሪቶች",
      table: {
        agency: "ድርጅት ስም",
        totalStaff: "ጠቅላላ ሰራተኞች",
        trained: "የሰለጠኑ",
        deployed: "የተሰማሩ",
        lastUpdate: "የመጨረሻ ዝመና",
        actions: "እርምጃዎች",
      },
      modal: {
        title: "የድርጅት HRMS ሪፖርቶች",
        subtitle: "የአስተዳደር ስርዓት",
        backToSelection: "ወደ ምርጫው ተመለስ",
        downloadPdf: "PDF አውርድ",
        generating: "መረጃዎችን በማዘጋጀት ላይ...",
        viewReport: "ሪፖርቱን ተመልከት",
      },
      reportTypes: {
        weekly: {
          name: "ሳምንታዊ አፈጻጸም",
          desc: "ሳምንታዊ የእንቅስቃሴ ማጠቃለያ እና ቁልፍ መለኪያዎች።",
        },
        monthly: {
          name: "የወርሃዊ አፈጻጸም",
          desc: "የድርጅቱ እንቅስቃሴዎች ስታቲስቲካዊ አጠቃላይ እይታ።",
        },
        yearly: {
          name: "ዓመታዊ አፈጻጸም",
          desc: "ዓመታዊ የአፈጻጸም ግምገማ እና የአዝማሚያ ትንተና።",
        },
        personnel: {
          name: "የሰራተኞች ሰነዶች",
          desc: "የጥበቃ ሰራተኞች የጀርባ ምርመራ እና ህጋዊ መታወቂያዎች።",
        },
        payroll: {
          name: "የደመወዝ እና ጥቅማጥቅሞች",
          desc: "ደመወዝ፣ ዋስትና እና የማህበራዊ ዋስትና ተገዢነት።",
        },
        training: {
          name: "የስልጠና ምስክር ወረቀቶች",
          desc: "የሁሉም የደህንነት ሰራተኞች የምስክር ወረቀት ሁኔታ።",
        },
      },
      success: "የHRMS ሪፖርቱ በተሳካ ሁኔታ ወርዷል!",
    },
    common: {
      submit: "አስገባ",
      cancel: "ሰርዝ",
      next: "ቀጣይ",
      back: "ተመለስ",
      upload: "ጫን",
      download: "አውርድ",
      search: "ፈልግ",
      filter: "አጣራ",
      loading: "በመጫን ላይ...",
      success: "ተሳክቷል",
      error: "ስህተት",
      edit: "አስተካክል",
      delete: "ሰርዝ",
      save: "አስቀምጥ",
      add: "ጨምር",
      status: "ሁኔታ",
      active: "ንቁ",
      inactive: "ቦዝኗል",
      suspended: "የታገደ",
    },
  },
};

export type Language = "en" | "am";
