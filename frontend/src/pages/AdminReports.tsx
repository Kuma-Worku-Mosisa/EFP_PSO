...
export const AdminReports = () => {
  ...
  const data = [
    { name: language === 'am' ? 'ጥር' : 'Jan', revenue: 4000, apps: 24 },
    { name: language === 'am' ? 'የካ' : 'Feb', revenue: 3000, apps: 13 },
    { name: language === 'am' ? 'መጋ' : 'Mar', revenue: 2000, apps: 98 },
    { name: language === 'am' ? 'ሚያ' : 'Apr', revenue: 2780, apps: 39 },
    { name: language === 'am' ? 'ግን' : 'May', revenue: 1890, apps: 48 },
    { name: language === 'am' ? 'ሰኔ' : 'Jun', revenue: 2390, apps: 38 },
  ];
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          <button className="flex items-center space-x-2 px-6 py-3 bg-white border rounded-xl font-bold text-gray-600 hover:bg-gray-50 transition-all">
            ...
            <span>{t.reports.last30Days}</span>
          </button>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-bold text-primary">{t.reports.revenueGrowth}</h3>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-bold text-primary">{t.reports.appVolume}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: t.reports.activeAgencies, value: "482", icon: <Shield className="text-primary" />, trend: `+5 ${t.reports.thisMonth}` },
          { label: t.reports.pendingRenewals, value: "12", icon: <TrendingUp className="text-amber-500" />, trend: t.reports.dueIn7Days },
          { label: t.reports.totalPersonnel, value: "12,482", icon: <Users className="text-purple-500" />, trend: t.reports.thisWeek },
        ].map((item, i) => (
