...
import { Loader2, Upload, Trash2, Save, Eye, CheckCircle2, AlertCircle, Settings as SettingsIcon } from "lucide-react";
...
export default function SystemSettings() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        setNotification({ type: "error", text: isAm ? "ከሰርቨር ጋር መገናኘት አልተሳካም።" : "Failed to connect to the server." });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const resp = await fetch("http://localhost:5000/api/system-settings/upload", { method: "POST", body: fd });
      const j = await resp.json();
      if (j.success && j.url) {
        setFormData((prev) => ({ ...prev, efpLogoUrl: j.url }));
        setNotification({ type: "success", text: isAm ? "ሎጎ ተሰቅሏል (ተቆጥቧል)። ለማስቀመጥ ያስቀምጡ።" : "Logo uploaded (staged). Save to persist." });
      } else {
        setNotification({ type: "error", text: j.message || (isAm ? "ሰቀላ አልተሳካም።" : "Upload failed.") });
      }
    } catch {
      setNotification({ type: "error", text: isAm ? "የኔትወርክ ስህተት ተከስቷል።" : "Upload network error." });
    } finally {
      setIsSaving(false);
  ...
  const handleRemoveLogo = () => {
    ...
    setNotification({ type: "success", text: isAm ? "ሎጎ ተወግዷል።" : "Staged logo cleared." });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        setNotification({ type: "success", text: isAm ? "የስርዓት ቅንብሮች በተሳካ ሁኔታ ተዘምነዋል።" : "System settings updated successfully." });
        if (result.data) {
          setFormData((prev) => ({
            ...prev,
            governmentName: result.data.governmentName || prev.governmentName,
            issuingAuthority: result.data.issuingAuthority || prev.issuingAuthority,
            efpLogoUrl: result.data.efpLogoUrl || prev.efpLogoUrl,
          }));
        }
      } else {
        setNotification({ type: "error", text: result.message || (isAm ? "ዝማኔ አልተሳካም።" : "Update failed.") });
      }
    } catch {
      setNotification({ type: "error", text: isAm ? "የኔትወርክ ስህተት ተከስቷል።" : "A network error occurred." });
    } finally {
      setIsSaving(false);
      setTimeout(() => setNotification(null), 3000);
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500 font-medium">{isAm ? "የውቅር ሞተር በመጫን ላይ..." : "Loading Configuration Engine..."}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">{isAm ? "ለፌዴራል ፖሊስ ዳሽቦርድ ዓለም አቀፍ የማንነት ተለዋዋጮችን ያስተዳድሩ።" : "Manage global identity variables for the Federal Police dashboard."}</p>
        </div>
      </div>

      {notification && (
        <div className={`flex items-center gap-2 p-4 rounded-2xl text-sm font-bold shadow-sm border ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-600 mb-1.5">
                {isAm ? "የሰጪ ባለስልጣን / ዳይሬክቶሬት" : "Issuing Authority / Directorate"}
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <div className="flex items-start gap-4">
                <div className="w-28 h-28 rounded-2xl bg-gray-50 flex items-center justify-center overflow-hidden border border-gray-100 shrink-0">
                  <img
                    src={formData.efpLogoUrl || "https://via.placeholder.com/112?text=LOGO"}
                    alt="EFP Logo"
                    className="w-full h-full object-contain"
                    onError={(e) => (e.currentTarget.src = "https://via.placeholder.com/112?text=LOGO")}
                  />
                </div>
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    {formData.efpLogoUrl ? (
                      ...
                      <span className="text-sm text-gray-400 font-medium">{isAm ? "ሎጎ አልተሰቀለም" : "No logo staged"}</span>
                    )}
                  </div>

                  <p className="text-xs text-gray-400">
                    {isAm ? "የሚመከሩ ቅርፀቶች: PNG, JPG, SVG። ሎጎው ካሬ ቅርፅ ያለው እንዲሆን ያድርጉ። ሰቅለው ያስቀምጡ።" : "Recommended formats: PNG, JPG, SVG. Keep logo square for best results. Upload stages the file; click \"Save Configuration\" to persist."}
                  </p>

                  {isSaving && (
                    <p className="text-xs text-gray-500 font-medium flex items-center gap-1">
                      <Loader2 className="w-3 h-3 animate-spin" /> {isAm ? "በመስቀል ላይ..." : "Uploading..."}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="pt-2">
              ...
              >
                ...
                {isSaving
                  ? isAm ? "ሰርቨሮችን በማመሳሰል ላይ..." : "Synchronizing Servers..."
                  : isAm ? "ውቅር አስቀምጥ" : "Save Configuration"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-[#003366]/20 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center justify-between border-b-2 border-gray-100 pb-4">
              <img
                src={formData.efpLogoUrl || "https://via.placeholder.com/80?text=LOGO"}
                alt="EFP Logo"
              ...
              <div className="text-right">
                <h3 className="text-xl font-black text-[#003366] uppercase tracking-wide">
                  {formData.governmentName || (isAm ? "የፌዴራል ፖሊስ" : "Federal Police")}
                </h3>
                <p className="text-sm font-bold text-[#003366]/70 mt-1">
                  {formData.issuingAuthority || (isAm ? "የዳይሬክቶሬት ስም" : "Directorate Name")}
                </p>
                <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest font-medium">
                  {isAm ? "የይፋዊ የደህንነት ኤጀንሲ የምስክር ወረቀት" : "Official Security Agency Certificate"}
                </p>
              </div>
            </div>

            <div className="py-8 text-center text-gray-300 font-mono text-sm">
              [ {isAm ? "የምስክር ወረቀት ይዘት እዚህ ይታያል" : "Certificate Body Content Renders Here"} ]
            </div>
          </div>

          <p className="text-xs text-gray-400 text-center">
            {isAm ? "ይህ ቅድመ-እይታ ራስጌው በህዝብ ማረጋገጫ ፖርታል እና በታተሙ PDFዎች ላይ እንዴት እንደሚፈጠር ያሳያል።" : "This preview shows exactly how the header will generate on the public verification portal and printed PDFs."}
          </p>
        </div>
      </div>
    </div>
