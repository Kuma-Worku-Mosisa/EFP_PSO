// filepath: frontend/src/pages/FAQ.tsx
...
export const FAQ = () => {
  ...
  const loadFaqs = async () => {
    ...
    setLoadError(null);
    try {
      const resp = await apiRequest<{ success: boolean; data: any[] }>("/faqs");
  ...
  const handleToggle = async (index: number, faq: any) => {
    ...
    // open -> increment hitCount on server and update local state
    try {
      ...
      setFaqs((prev) =>
        prev.map((p) =>
          p.id === updated.id
            ? {
                ...p,
                hitCount: updated.hitCount ?? p.hitCount,
                q: updated.questionText ?? p.q,
                a: updated.answerText ?? p.a,
              }
            : p,
        ),
      );
    } catch (err) {
      // non-fatal: keep current UI but don't block opening
      // optionally could show a toast; keep silent for now
    }
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-12">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {["all", "general", "licensing", "technical", "payment"].map(
              (cat) => {
          ...
          <AnimatePresence mode="popLayout">
            {filteredFaqs.map((faq: any, i: number) => {
              ...
              return (
                ...
                >
                  <button
                    onClick={() => void handleToggle(i, faq)}
                  ...
                  >
                    <div className="flex items-center space-x-6">
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                          isOpen
                            ? "gold-gradient text-primary rotate-12 shadow-lg shadow-secondary/20"
                            : "bg-gray-100 text-gray-400 group-hover:bg-white group-hover:shadow-lg"
                        }`}
                      >
                      ...
                      <div className="space-y-1">
                        <span
                          className={`text-xs font-black uppercase tracking-widest ${isOpen ? "text-secondary/80" : "text-gray-400"}`}
                        >
                          {categories[faq.cat]}
                        </span>
                        <h4
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="inline-flex items-center space-x-2 text-xs text-gray-500 mr-2">
                      ...
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 ${
                          isOpen
                            ? "bg-primary text-secondary rotate-180 scale-110 shadow-lg shadow-primary/20"
