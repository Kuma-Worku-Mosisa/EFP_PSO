//frontend/src/pages/FormalLetter.tsx
      ...
      >
        <div className="p-6 border-b flex items-center justify-between bg-white z-10">
          <div className="flex items-center space-x-4">
            <div>
              <h3 className="font-bold text-primary">{fileInfo.name}</h3>
...
export const FormalLetter = () => {
  ...
  const mapStatus = (backendStatus?: string | null) => {
    if (!backendStatus) return "none";
  ...
  React.useEffect(() => {
    if (file) {
      ...
      setPreviewUrl(url);
      setPreviewFileInfo({ name: file.name, size: file.size, type: file.type });
    ...
    if (remoteUrl) {
      ...
      setPreviewFileInfo({ name, size: 1, type: getFileTypeFromName(name) });
      return;
    }
  ...
  React.useEffect(() => {
    const loadFormalRequest = async () => {
      ...
      try {
        ...
        if (statusCode !== 404) {
          showToast("error", "Failed to load formal request status.");
        }
      } finally {
        setLoading(false);
      }
  ...
  const t_letter = {
    en: {
      ...
      pendingDesc:
        "Your formal letter has been submitted and is currently being reviewed by the Federal Police Admin. You will be notified once it is approved.",
      pendingStatus: "Status: Pending Review",
      approvedTitle: "Request Approved!",
      approvedDesc:
        "Your formal letter of request has been approved. You can now proceed with the full license application.",
      approvedBtn: "Go to New Application",
      rejectedTitle: "Request Rejected",
      rejectedDesc:
        "Unfortunately, your formal letter of request was not approved. Please review the requirements and submit a corrected letter.",
      rejectedBtn: "Try Again",
      guidelinesTitle: "Official Guidelines & Requirements",
      guidelinesDesc:
        "What the formal letter of request must fulfill for the Federal Police Commission.",
      important: "Important Note",
      importantDesc:
        "The formal letter of request must be on your agency's official letterhead, signed by the manager, and stamped. It should clearly state the purpose of the application and the type of security services you intend to provide.",
    },
    am: {
      title: "የማመልከቻ ደብዳቤ",
      ...
      pendingDesc:
        "መደበኛ ደብዳቤዎ ገብቷል እና በአሁኑ ጊዜ በፌዴራል ፖሊስ አስተዳዳሪ እየተገመገመ ነው። ሲፈቀድ ማሳወቂያ ይደርስዎታል።",
      ...
      rejectedDesc:
        "እንደ አለመታደል ሆኖ መደበኛ የማመልከቻ ደብዳቤዎ አልጸደቀም። እባክዎ መስፈርቶቹን ይገምግሙ እና የተስተካከለ ደብዳቤ ያቅርቡ።",
      rejectedBtn: "እንደገና ይሞክሩ",
      guidelinesTitle: "ይፋዊ መመሪያዎች እና መስፈርቶች",
      guidelinesDesc: "መደበኛ የማመልከቻ ደብዳቤ ለፌዴራል ፖሊስ ኮሚሽን ማሟላት ያለባቸው ነገሮች።",
      important: "ጠቃሚ ማሳሰቢያ",
      importantDesc:
        "መደበኛ የማመልከቻ ደብዳቤው በተቋምዎ ይፋዊ የደብዳቤ ራስ (letterhead) ላይ መሆን አለበት፣ በስራ አስኪያጁ ተፈርሞ እና ማህተም ተደርጎበት። የማመልከቻውን ዓላማ እና ለማቅረብ ያሰቡትን የጥበቃ አገልግሎት አይነት በግልፅ መግለጽ አለበት።",
    },
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!file || !user?.id) {
      showToast("error", "Please select a file before submitting.");
      return;
    }
    ...
    try {
      ...
      formData.append("userName", user.fullName || user.username || "user");
      formData.append("year", String(new Date().getFullYear()));
      formData.append("requestLetter", file);

      const uploadResponse = await fetch(`${API_BASE}/formal-requests/upload`, {
        method: "POST",
      ...
      const requestLetterUrl = uploadData?.data?.requestLetterUrl;
      if (!requestLetterUrl) {
        throw new Error("Upload succeeded but no URL returned");
      }

      await apiRequest("/formal-requests", {
        method: "POST",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      ...
      showToast("success", "Formal request submitted successfully.");
    } catch (error: any) {
    ...
    <div className="max-w-4xl mx-auto space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div
              className={cn(
                ...
                openedForEdit &&
                  "border-amber-400 border-dashed bg-amber-50/20 ring-4 ring-amber-50 animate-pulse",
              )}
            >
              {file && (
              ...
              >
                {openedForEdit ? (
                  ...
                  <Upload className="w-10 h-10 group-hover:bounce" />
                )}
              </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex space-x-3">
                  <span className="flex-shrink-0 w-6 h-6 bg-secondary/20 text-secondary rounded-full flex items-center justify-center text-xs font-bold">
                    {i + 1}
                  </span>
          ...
          <div className="space-y-6">
            <div className="flex items-center space-x-3 border-b pb-4">
              <span className="w-10 h-10 bg-secondary text-primary rounded-xl flex items-center justify-center font-bold">
                አማ
              </span>
