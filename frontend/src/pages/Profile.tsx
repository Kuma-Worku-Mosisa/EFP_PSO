//filepath: frontend/src/pages/Profile.tsx
...
export const Profile = () => {
  const { user, updateAvatar, updateUserProfile } = useAuth() as any;
  ...
  const handleSave = () => {
    // Password changes are handled via the Change Password modal
    setIsSaving(true);
    // Build fullName from parts
    const fullName =
      `${formData.firstName} ${formData.middleName} ${formData.lastName}`.trim();

    // Call backend to update profile via auth context
    (async () => {
      try {
        ...
        setIsSaving(false);
        const message =
          err?.message || (isAm ? "አገልግሎት አልተገኘም" : "Service unavailable");
