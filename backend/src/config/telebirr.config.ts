// file: backend/src/config/telebirr.config.ts



// 1. Get the key from env, or fall back to your specific Sandbox private key string

const rawKey = 

  process.env.TELEBIRR_PRIVATE_KEY || 

  "";



// 2. Clean out raw double escapes common in Windows env parsers

let formattedKey = rawKey.replace(/\\n/g, "\n").trim();



// 3. Auto-wrap with proper PKCS#8 headers if they aren't fully formed

if (formattedKey && !formattedKey.startsWith("-----BEGIN PRIVATE KEY-----")) {

  formattedKey = `-----BEGIN PRIVATE KEY-----\n${formattedKey}\n-----END PRIVATE KEY-----`;

}



export const telebirrConfig = {

  // Use the developerportal base for Token and Gateway

  baseUrl: "https://developerportal.ethiotelebirr.et:38443/apiaccess/payment/gateway",

  webBaseUrl: "https://developerportal.ethiotelebirr.et:38443/payment/web/paygate?",

  fabricAppId: process.env.TELEBIRR_APP_ID || "1227525300787308",

  merchantAppId: process.env.TELEBIRR_MERCHANT_APP_ID || "1227525300787308",

  merchantCode: process.env.TELEBIRR_MERCHANT_CODE || "167374",

  appSecret: process.env.TELEBIRR_APP_SECRET || "fad0f06383c6297f545876694b974599",

  privateKey: formattedKey,

  otherParams: "&version=1.0&trade_type=Checkout",

};